...
public class PlayerPickup : MonoBehaviour
{
    ...
    {
        Collider[] colliders = Physics.OverlapSphere(transform.position, detectionRadius, ~0, QueryTriggerInteraction.Collide); //구체적 충돌체를 만든다
        PickupItem closestItem = null;
        float closestDistance = Mathf.Infinity;

        foreach (Collider itemCollider in colliders) //충돌 구체 링크 들어가면서 검사
        {
            ...
            if (distance < closestDistance) //가장 가까운 오브젝트를 찾는다.
            {
    ...
    private IEnumerator PickupAnimation()
    {
        ...
        // 이동 Blend Tree로 돌아간다.
        animator.CrossFade("Blend Tree", 0.11f);
